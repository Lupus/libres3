...
type op = {
  ...
  with_private_settings : url -> max_wait:float -> (string -> string option Lwt.t) -> string -> unit Lwt.t;
...
let with_private_settings (`Url src) ~max_wait f key =
  (ops_of_url src).with_private_settings src ~max_wait f key
...
module RegisterURLScheme(O: SchemeOps) = struct
  ...
  let ops = {
    ...
    get_meta = O.get_meta;
    set_meta = O.set_meta;
    with_private_settings = O.with_private_settings;
